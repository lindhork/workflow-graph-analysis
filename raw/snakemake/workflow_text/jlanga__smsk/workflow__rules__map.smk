rule map_bwa_index:
    ...
    input:
        RAW + "genome.fa",
    output:
        expand(RAW + "genome.fa.{extension}", extension="amb ann bwt pac sa".split(" ")),
    log:
        MAP + "bwa_index.log",
    conda:
        "map.yml"
...
def get_sample(wildcards):
    ...
    path = samples[samples.sample_id == wildcards.sample][["path"]].values.tolist()[0]
    return path
...
rule map_bwa:
    ...
    input:
        ...
        sample=get_sample,
        ref_files=expand(
            RAW + "genome.fa.{extension}", extension="amb ann bwt pac sa".split(" ")
        ),
    output:
...
rule map:
    ...
    input:
        expand(MAP + "{sample}.sorted.bam.bai", sample=SAMPLES),
